		  ...
		  if ( $('[id*=apus-twitter<?php echo esc_js( $user ); ?>]').length ) {
		   $('#apus-twitter<?php echo esc_js( $user ); ?> iframe').each( function(){
		    ...
		    if ( ibody.find( '.timeline-Body .timeline-TweetList li.timeline-TweetList-tweet' ).length ) {
				...
				if(show_scroll == 1){
					ibody.find( '.timeline-Body .timeline-TweetList' ).css( 'max-height', height );
					ibody.find( '.timeline-Body .timeline-TweetList' ).css( 'overflow-y', 'auto' );	
					ibody.find( '.timeline-Body .twitter-timeline' ).css( 'height', 'inherit !important' );	
				}
		    } else {
		     $(this).hide();
		    }
